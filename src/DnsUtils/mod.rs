...
use crate::dns_type::{DnsType, DnsTypes};
    ...
    impl ResourceRecord<'_> {
        fn new(buffer: &[u8]) -> (ResourceRecord, usize) {
            ...
            let domain_class = ((buffer[len + 2] as u16) << 8) + (buffer[len + 3] as u16);
            let ttl = ((buffer[len + 4] as u32) << 24) + ((buffer[len + 5] as u32) << 16) + ((buffer[len + 6] as u32) << 8) + (buffer[len + 7] as u32);
        ...
        fn rdata_to_string(&self) -> String {
            match self.domain_type {
                DnsTypes::A   =>  {
                    // rdata is ipv4
                    let ipv4 = self.rdata[0].to_string() + "." + &self.rdata[1].to_string() + "." + &self.rdata[2].to_string() + "." + &self.rdata[3].to_string();
                    ipv4
                },
                DnsTypes::CNAME   =>  {
